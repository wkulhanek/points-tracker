...
**Email sending** (`internal/email/`): `sender.go` defines a single `Sender` interface; `email/factory` reads the `email_settings` singleton row's `provider_type` and returns the `email/smtp` implementation (currently the only provider). Both the notification scheduler and the "Test email" button send through this one interface.
...
See `SECURITY.md` for the full security posture and what's intentionally deferred (e.g. plaintext SMTP credentials at rest, no multi-user support) versus what's still recommended before wider exposure.
